...
2) Adding: the contribution's claims are added, each checked to be dated `created_at` $<= t$ and to use no type reserved to the Sequencer: limiting, branch-table, and history claims are the Sequencer's alone.
