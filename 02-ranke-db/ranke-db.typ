...
`output` shapes each result along five axes: `shape` sets what an element is — `single`, one reached endpoint, or `path`, its whole route; `detail` how much it carries — the `id` alone, or the `claims` themselves; `form` which field values a claim carries — `original` as stored, or `materialized` with any diff chain resolved over its predecessor; `content` how many bytes are inlined per claim, absent inlining none and a cap of `0` inlining every claim in full, with `overflow` deciding what happens past a cap — `cutoff` or `omit`; and `encoding` the serialization of each result — `json` (@rfc7464), text with content base64-encoded, or `cbor` (@rfc8742), binary. The stream frames them as a sequence in either encoding.
